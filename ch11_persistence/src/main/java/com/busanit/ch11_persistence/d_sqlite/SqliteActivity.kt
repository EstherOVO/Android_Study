...
//                  성공할 경우 행의 ID, 실패할 경우 -1 반환
                    ...
                    Toast.makeText(this@SqliteActivity, "나이를 입력해 주세요.", Toast.LENGTH_SHORT).show()
                }
            }
...
//              문자열을 담을 빌더
                val strungBuilder = StringBuilder()
...
//                      getColumnIndex 컬럼의 순서를 가져온다.
                        val id = getInt(getColumnIndexOrThrow("id"))
                        val name = getString(getColumnIndexOrThrow("name"))
                        val age = getInt(getColumnIndexOrThrow("age"))

                        strungBuilder.append("아이디 : ${id}, 이름 : ${name}, 나이 : ${age}")
                    }
                }
...
//              결과 데이터 표시
                val data = strungBuilder.toString()
                textView.text = data
            }
        }
    }
